...
beforeEach(() => {
    ...
    createSocketSpy = vi
        .spyOn(websocket as any, "createSocket")
        .mockImplementation(() => {
            return {
    ...
    vi.spyOn(Card, "create").mockImplementation(async (color, type) =>
        ({ color, type } as unknown as Card)
    );
...
describe("WebSocketHandle basic tests", () => {
    ...
    it("reconnect sets correct URL", () => {
        ...
        expect(createSocketSpy).toHaveBeenCalled();
        expect(websocket.getUrl()).toBe(
            `ws://${MnauConfig.ip}:${MnauConfig.port}/game?user=Bob&reconnect=true`
        );
    });
});
...
describe("WebSocketHandle additional commands tests", () => {
    it("drawCardCommand sends DRAW move", () => {
        ...
        expect(sendSpy).toHaveBeenCalledWith(
            JSON.stringify({ requestType: "MOVE", move: { moveType: "DRAW" } })
        );
    });
    ...
    it("playPassCommand sends PASS move", () => {
        ...
        expect(sendSpy).toHaveBeenCalledWith(
            JSON.stringify({ requestType: "MOVE", move: { moveType: "PASS" } })
        );
    });
